...
def locate_library_dir(directory):
...
def insert_into_build_gradle(project, dep):
	...
	for line in data:
		if count == 0 and line.lstrip().startswith('dependencies') and line.rstrip().endswith('{'):
			data.insert(data.index(line) + 1, "\tcompile project(':deps:" + dep.extended_name + "')\n")
		if '{' in line:
			count += 1
		if '}' in line:
			count -= 1
...
def update_dep_for_project(project, dep):
	...
	with open(os.path.join(project.root, 'deps', dep.extended_name, 'build.gradle')):
		data = f.readlines()
